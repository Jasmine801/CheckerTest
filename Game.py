...
class Game:
    def __init__(self,
        ...
        for i in range(0, 6):
            if i % 2 == 0:
                ...
                x = self.kx * 5
            for j in range(0, 5):
                self.objects[(x, y)] = Computer_checkers(x, y, rad,
                                                         (255, 0, 0))
                x += self.kx * 6
                self.computer_c += 1
            y += self.ky
    ...
    def handle_mouse_down(self, pos):
        ...
        if self.bounds.collidepoint(pos):
            ...
            if str(type(self.objects.get((x_pos,
                                          y_pos)))) \
                    == "<class 'logic.Player_checkers.Player_checkers'>":
            ...
            if self.active_checker.checkers_type == "king" \
                    and (math.fabs(x_pos - cx_pos) >= self.kx * 3
                         and math.fabs(y_pos - cy_pos) >= self.ky
                         and math.fabs(x_pos - cx_pos) // (
                                 self.kx * 3) == math.fabs(
                        y_pos - cy_pos) // self.ky
                         or x_pos - cx_pos == 0
                ...
                if len(self.enemy_dict) == 0:
                    if self.objects.get((x_pos, y_pos)) is None:
                        ...
                        self.turn = "Computer"
                    return
                gen_finder = self.find_enemy_for_king_on_path(x_pos, y_pos,
                                                              cx_pos, cy_pos)
                if self.enemy_dict.get((x_pos, y_pos)) is None and next(
                        gen_finder):
                    self.move_checker((x_pos - cx_pos) // 5,
                                      (y_pos - cy_pos) // 5, x_pos, cx_pos,
                                      y_pos, cy_pos)
            ...
            if math.fabs(x_pos - cx_pos) == self.kx * 3 and math.fabs(
                    y_pos - cy_pos) == self.ky \
                    or x_pos - cx_pos == 0 \
                    and math.fabs(y_pos - cy_pos) == self.ky * 2:
                if len(self.enemy_dict) == 0:
                    if self.objects.get((x_pos, y_pos)) is None:
                        if y_pos - cy_pos <= 0:
                            self.move_checker(
                                (x_pos - cx_pos) // 5, (y_pos - cy_pos) // 5,
                                x_pos, cx_pos, y_pos, cy_pos)
                            if y_pos <= self.ky * 2:
                ...
                if self.enemy_dict.get((x_pos, y_pos)) is not None:
                    ...
                    if self.objects.get((new_x_pos, new_y_pos)) is None:
                        self.move_checker(
                            2 * (x_pos - cx_pos) // 5,
                            2 * (y_pos - cy_pos) // 5, new_x_pos, cx_pos,
                            new_y_pos, cy_pos)
            ...
            if self.enemy_dict.get((x_pos, y_pos)) is not None:
                if cx < x_pos < x and cy < y_pos < y \
                    ...
                    if self.objects.get((pos_x + kx * self.kx * 3,
                        ...
                        checker_flag = True
        if enemy_flag and not checker_flag:
            yield True
            yield (pos_x, pos_y)
        else:
            yield False
    ...
    def check_enemy_for_checker(self, x_pos, y_pos):
        ...
        for i in range(-1, 2):
            for j in range(-1, 2):
                ...
                if type(
                        possible_enemy) is not this_check_type \
                        and possible_enemy is not None:
                    if self.objects.get((x_pos + i * 2 * self.kx * 3,
                                         y_pos + j * 2 * self.ky)) is None:
                        if x_pos + i * 2 * self.kx * 3 in range(2, 773) \
                                and y_pos + j * 2 * self.ky in range(2, 798):
                            self.enemy_dict[
                                x_pos + i * self.kx * 3,
                                y_pos + j * self.ky] = possible_enemy
        for i in range(-1, 2):
            if i == 0:
                continue
            ...
            if type(
                ...
                if self.objects.get(
                    ...
                    if y_pos + i * 2 * self.ky * 2 in range(2, 798):
                        self.enemy_dict[
                            x_pos, y_pos + i * self.ky * 2] = possible_enemy
            ...
            for ky in range(-1, 2):
                ...
                while self.bounds.collidepoint((x_pos, y_pos)):
                    ...
                    if type(self.objects.get(
                            (x_pos, y_pos))) is this_check_type:
                        break
                    if type(self.objects.get(
                            (x_pos, y_pos))) is not this_check_type \
                            and self.objects.get((x_pos, y_pos)) is not None:
                        if self.objects.get((
                                x_pos + self.kx * 3 * kx,
                                y_pos + self.ky * ky)) is not None:
                            break
                        if self.objects.get((x_pos + self.kx * 3 * kx,
                                             y_pos + self.ky * ky)) is None \
                                and (self.objects.get((
                                x_pos - self.kx * 3 * kx,
                                y_pos - self.ky * ky)) is None
                                     or x_pos - self.kx * 3 * kx == x
                                     and y_pos - self.ky * ky == y) \
                                and self.bounds.collidepoint((
                                x_pos + self.kx * 3 * kx,
                                y_pos + self.ky * ky)):
        ...
        for ky in range(-1, 2):
            ...
            while self.bounds.collidepoint((x_pos, y_pos)):
                ...
                if type(self.objects.get(
                        ...
                        and self.objects.get((x_pos, y_pos)) is not None:
                    if self.objects.get(
                            (x_pos, y_pos + 2 * self.ky * ky)) is not None:
                        break
                    if self.objects.get(
                            (x_pos, y_pos + 2 * self.ky * ky)) is None \
                            and (self.objects.get((
                            x_pos, y_pos - 2 * self.ky * ky)) is None
                                 or y_pos - 2 * self.ky * ky == y) \
                            and self.bounds.collidepoint((
                            x_pos, y_pos + 2 * self.ky * ky)):
                        self.enemy_dict[(x_pos, y_pos)] = self.objects.get(
                            (x_pos, y_pos))
                        break
    ...
    def computer_turn(self):
        """
        makes an opponent move
        :return:
        """
        comp_str = "<class 'logic.Computer_checkers.Computer_checkers'>"
        self.check_to_enemy(
            comp_str)
        if len(self.enemy_dict) == 0:
            for pos, checker in self.objects.items():
                if str(type(checker)) == comp_str:
                    x_pos, y_pos = pos
                    self.active_checker = self.objects.get((x_pos, y_pos))

                    if self.objects.get((x_pos, y_pos + 2 * self.ky)) is None \
                            and self.bounds.collidepoint((
                            x_pos, y_pos + 2 * self.ky)):
                        self.move_checker(0, 2 * self.ky // 5, x_pos, x_pos,
                                          y_pos + 2 * self.ky, y_pos)
                        self.turn = "Player"
                        return

                    for kx in range(-1, 2):
                        if kx != 0:
                            if self.objects.get((x_pos + kx * self.kx * 3,
                                                 y_pos + self.ky)) is None \
                                    and self.bounds.collidepoint((
                                    x_pos + kx * self.kx * 3,
                                    y_pos + self.ky)):
                                self.move_checker((kx * self.kx * 3) // 5,
                                                  self.ky // 5,
                                                  x_pos + kx * self.kx * 3,
                                                  x_pos, y_pos + self.ky,
                                                  y_pos)
                                if y_pos + self.ky >= 800 - self.ky * 2:
                                    self.active_checker.color = (255, 40, 40)
                                    self.active_checker.checkers_type = "king"
                                self.turn = "Player"
                                return
                            for ky in range(-1, 2):
                                if ky != 0:
                                    if self.active_checker.checkers_type \
                                            == "king":
                                        if self.objects.get((
                                                x_pos + kx * self.kx * 3,
                                                y_pos + ky * self.ky)) \
            ...
            return
        if len(self.enemy_dict) != 0:
            pos, checker = self.enemy_dict.popitem()
            x_pos, y_pos = pos
            for kx in range(-1, 2):
                for ky in range(-1, 2):
                    if ky == 0:
                    ...
                    if kx == 0:
                        ky *= 2
                    gen_finder = self.try_to_find_king(x_pos, y_pos, kx, ky)
                    if next(gen_finder):
                        cx_pos, cy_pos = next(gen_finder)
                        self.active_checker = self.objects.get(
                            (cx_pos, cy_pos))
                        self.move_checker(
                            (x_pos - kx * self.kx * 3 - cx_pos) // 5,
                            (y_pos - ky * self.ky - cy_pos) // 5,
                            x_pos - kx * self.kx * 3, cx_pos,
                            y_pos - ky * self.ky, cy_pos)
                        self.objects[(x_pos, y_pos)] = None
                        self.player_c -= 1
                        self.enemy_dict.clear()
                        self.check_to_enemy(
                    ...
                    if str(type(self.objects.get(
                            (x_pos + kx * self.kx * 3,
                             y_pos + ky * self.ky)))) == comp_str:
                        if self.objects.get((x_pos - kx * self.kx * 3,
                                             y_pos - ky * self.ky)) is None:
                            self.active_checker = self.objects.get((
                                x_pos + kx * self.kx * 3,
                                y_pos + ky * self.ky))
                            self.move_checker(2 * -kx * 3 * self.kx // 5,
                                              2 * -ky * self.ky // 5,
                                              x_pos - kx * self.kx * 3,
                                              x_pos + kx * self.kx * 3,
                                              y_pos - ky * self.ky,
                                              y_pos + ky * self.ky)
                            if y_pos - ky * self.ky >= 800 - 2 * self.ky:
                                self.active_checker.color = (255, 40, 40)
                                self.active_checker.checkers_type = "king"
                            self.objects[(x_pos, y_pos)] = None
                            self.player_c -= 1
                            self.enemy_dict.clear()
                            self.check_to_enemy(comp_str)
                            if len(self.enemy_dict) == 0:
                                self.active_checker = None
                                self.turn = "Player"
                            self.enemy_dict.clear()
    ...
    def prepare_tuple_to_send(self, tup):
        res = str(tup).replace(' ', '-')
        res = res.replace(',', '')
        res = res.replace('(', '')
        res = res.replace(')', '')
    ...
    def run_heroku(self):
        heroku_conn = heroku3.from_key('45acf494-8513-4730-9e99-55dd37356297')
        for a in heroku_conn.apps():
            a.delete()
        app = heroku_conn.create_app('checker-game')
        config = app.config()
        config['0'] = '0'
        flag = True
        while not self.game_over:
            if not flag:
                ...
                if config['0'] != '0':
                    getted_str = config['0']
                    getted_str = getted_str.split('-')
                    from_x = 775 - int(getted_str[0])
                    from_y = 800 - int(getted_str[1])
                    to_x = 775 - int(getted_str[2])
                    to_y = 800 - int(getted_str[3])
            ...
            if flag:
                ...
                if self.from_c != tuple():
                    from_c = self.prepare_tuple_to_send(self.from_c)
                    to_c = self.prepare_tuple_to_send(self.to_c)
                    self.from_c = tuple()
                    self.to_c = tuple()
                    if self.turn == "Player":
                        config['1'] = from_c + '-' + to_c + '-1'
                    if self.turn == "Computer":
                        config['1'] = from_c + '-' + to_c + '-0'
                        self.turn = "Player"
                        flag = False
    ...
    def run_computer(self):
        while not self.game_over:
            self.surface.blit(self.background_image, (0, 0))
            if self.player_c == 0 or self.computer_c == 0:
                self.game_over = True
                continue
            if self.turn == "Player":
                self.handle_events()
